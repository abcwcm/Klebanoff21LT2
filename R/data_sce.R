#' Load the SCE for the full SCE data set including cells without V(D)J info
#'
#' @description Load the SingleCellExperiment object that holds the SCE that
#' only filtered for low-quality genes and cells; irrespective of V(D)J
#' information.
#'
#' @usage sce.filt <- load_21LT2filt()
#' @seealso \code{\link{load_21LT2filt}}, \code{\link{load_21LT2filt}}
#' @return an SCE object that needs to be assigned to an object in the environment
#'
#' @export
#'
load_21LT2filt <- function(){
    out <- load_sce(which_assays = "all", sample = "21LT2")
...
#' reduced dimensionality results (MUT/WT = batch) based on all cells;
#' irrespective of the V(D)J information.
#' @return an SCE object that needs to be assigned to an object in the environment
#' @usage sce.merged <- load_0606T1merged()
#' @export
#'
load_21LT2merged <- function(){
    out <- load_sce(which_assays = "reconstructed", sample = "21LT2Merged")
    return(out)
}

#' Load the SCE for those cells that have a "shared" clonotype
#'
#' @description Load the SingleCellExperiment object that holds the cells
#  with clonotypes that are present in both conditions.
...
#' @return an SCE object that needs to be assigned to an object in the environment
#' @usage sce.shared <- load_21LT2filt()
#' @export
#'
load_21LT2shared <- function(){
    out <- load_sce(which_assays = "logcounts", sample = "21LT2Shared")
    return(out)
}
...
#' Filtered cells
#'
#' @format Named list of cell numbers following the filtering steps described in this vignette.
"cell_filt"


#' Filtered genes
#'
#' @format Named list of gene numbers following the filtering steps described in this vignette.
"gene_filt"
...
load_sce <- function(which_assays = "all", sample = "Sample", ...){
    ...
    fn <- paste0("sce_storage_", sample, ".txt")
    fl <- system.file("extdata", fn, package = "Klebanoff21LT2")
    if(fl == ""){stop(paste(fn, "does not exist in package 'Klebanoff21LT2'."))}
